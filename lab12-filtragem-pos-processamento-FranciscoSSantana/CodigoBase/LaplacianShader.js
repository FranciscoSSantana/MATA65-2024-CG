...
const LaplacianShader = {
	...
	fragmentShader: /* glsl */`
		...
		// fetch the 3x3 neighbourhood of a fragment
			...
			switch (canalDeCor) {
				...
				case 4: //all colors
					...
					float valueLR = L[0][0] * x0y0.r + L[1][0] * x1y0.r + L[2][0] * x2y0.r +
                           			L[0][1] * x0y1.r + L[1][1] * x1y1.r + L[2][1] * x2y1.r +
                           			L[0][2] * x0y2.r + L[1][2] * x1y2.r + L[2][2] * x2y2.r;

					float valueLG = L[0][0] * x0y0.g + L[1][0] * x1y0.g + L[2][0] * x2y0.g +
                           			L[0][1] * x0y1.g + L[1][1] * x1y1.g + L[2][1] * x2y1.g +
                           			L[0][2] * x0y2.g + L[1][2] * x1y2.g + L[2][2] * x2y2.g;
					
					float valueLB = L[0][0] * x0y0.b + L[1][0] * x1y0.b + L[2][0] * x2y0.b +
                           			L[0][1] * x0y1.b + L[1][1] * x1y1.b + L[2][1] * x2y1.b +
                           			L[0][2] * x0y2.b + L[1][2] * x1y2.b + L[2][2] * x2y2.b;
					
					gl_FragColor = vec4( vec3( max(valueLR, 0.0), max(valueLG, 0.0), max(valueLB, 0.0) ), 1 );
					return;
					break;
				default:
					break;
		...
		// convolution
            ...
            float valueL = L[0][0] * tx0y0 + L[1][0] * tx1y0 + L[2][0] * tx2y0 +
                           L[0][1] * tx0y1 + L[1][1] * tx1y1 + L[2][1] * tx2y1 +
                           L[0][2] * tx0y2 + L[1][2] * tx1y2 + L[2][2] * tx2y2;

            if(valueL < 0.0)
                gl_FragColor = vec4( vec3( 0.0 ), 1 );
            else
                gl_FragColor = vec4( vec3( valueL ), 1 );
		}`
